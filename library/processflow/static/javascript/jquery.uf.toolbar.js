define(['app', 'processflow', 'processflow-tree'], function( app, processflow ) {
  $.widget( "processflow.uf_toolbar", {
    options: {
    ...
    _create: function() {
      ...
      this.els.trees.workspace.on('select_node.jstree', function(node, selectedInfos, event) {
        ...
        if (processflow.isOperatorDefined(selected)) {
          processflow.openProcess(selected);
        }
      });
    ...
    refreshTrees: function() {
      var self = this;
      processflow.getOperators(function(data) {
